...
export class UpdateMissionComponent implements OnInit {
  ...
  FetchDetail(id: any) {
    this.service.MissionDetailById(id).subscribe((data: any) => {
      ...
      let startDateformat = this.datePipe.transform(
        this.editData.startDate,
        'yyyy-MM-dd'
      );
      this.editData.startDate = startDateformat;
      let endDateformat = this.datePipe.transform(
        this.editData.endDate,
        'yyyy-MM-dd'
      );
      this.editData.endDate = endDateformat;
      let registrationDeadLineDateformat = this.datePipe.transform(
        this.editData.registrationDeadLine,
        'yyyy-MM-dd'
      );
      ...
      this.editMissionForm = this.fb.group({
        id: [this.editData.id],
        missionTitle: [
          this.editData.missionTitle,
          Validators.compose([Validators.required]),
        ],
        missionDescription: [
          this.editData.missionDescription,
          Validators.compose([Validators.required]),
        ],
        countryId: [
        ...
        totalSheets: [
          this.editData.totalSheets,
          Validators.compose([Validators.required]),
        ],
        missionThemeId: [
          this.editData.missionThemeId,
          Validators.compose([Validators.required]),
        ],
        missionSkillId: [
          this.editData.missionSkillId.split(','),
          Validators.compose([Validators.required]),
        ],
        missionImages: [''],
      });
      ...
      if (this.editData.missionImages) {
        ...
        for (const photo of this.imageUrl) {
          this.imageListArray.push(
            this.service.imageUrl + '/' + photo.replaceAll('\\', '/')
          );
        }
      }
    });
    ...
    if (event.target.files.length > 0) {
      ...
      for (const file of files) {
        ...
        reader.onload = (e: any) => {
          ...
          this.formData.append('missionImages', file);
        };
  ...
  OnRemoveImage(index: number) {
    ...
    const formDataImages = this.formData.getAll('missionImages');
    formDataImages.splice(index, 1);
    this.formData.delete('missionImages');
    formDataImages.forEach((image) => {
      this.formData.append('missionImages', image);
    });
  }

  OnCancel() {
    this.router.navigate(['/admin/manage-missions']);
  }
  ...
  async OnSubmit() {
    ...
    let updateImageUrl = '';
    var SkillLists = Array.isArray(value.missionSkillId)
